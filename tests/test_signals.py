...
def test_all_news_sources_unavailable_sets_unknown_and_penalizes_confidence() -> None:
    strong_uptrend = [1 + i / 1000 for i in range(20)]
    complete = build_hint(snapshot(strong_uptrend), [], 0.5)
    unavailable = build_hint(
    ...
    assert unavailable.news_coverage is NewsCoverage.UNAVAILABLE
    assert unavailable.failed_news_sources == 4
    assert unavailable.confidence == complete.confidence - 18
    assert any("UNKNOWN" in reason for reason in unavailable.reasons)


def test_one_failed_news_source_only_applies_small_penalty() -> None:
    strong_uptrend = [1 + i / 1000 for i in range(20)]
    complete = build_hint(snapshot(strong_uptrend), [], 0.5)
    partial = build_hint(
        snapshot(strong_uptrend),
    ...
    assert partial.news_risk is NewsRisk.LOW
    assert partial.news_coverage is NewsCoverage.PARTIAL
    assert partial.confidence == complete.confidence - 2
